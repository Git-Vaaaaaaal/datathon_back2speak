...
HIGH_CENTROID_SH_TO_S = 3500.0
VERY_HIGH_CENTROID_SH_TO_S = 3800.0
...
app = FastAPI(title="French Pronunciation Analyzer V7", version="7.0")
...
class FinalDecision(BaseModel):
    status: str  # correct / acceptable / incorrect
    message: str
        ...
        for item in updated:
            if (
                ...
                if item.expected == "ʃ" and check.acoustic_guess == "s":
                    if check.confidence >= ACOUSTIC_DISAGREE_THRESHOLD:
                        if item.status == "correct":
                            item.status = "uncertain"

                    if (
                        check.confidence >= 0.6
                        and check.spectral_centroid_hz >= VERY_HIGH_CENTROID_SH_TO_S
                    ):
                        item.status = "mismatch"

                    if (
                        check.spectral_centroid_hz >= HIGH_CENTROID_SH_TO_S
                        and item.status == "correct"
                    ):
                        item.status = "uncertain"
    ...
    for span in spans:
        ...
        for p in word_items:
            ...
            if p.status == "mismatch":
                ...
                issue = "missing_or_swallowed"
            elif p.status == "inserted":
                issue = "extra_sound"
            else:
                issue = "unclear_or_crushed"
        ...
        if not word_errors and not word_uncertain and not is_crushed:
            ...
            if word_errors:
                for err in word_errors:
                    if err.status == "mismatch":
                        messages.append(f'{err.expected} → {err.predicted}')
                    elif err.status == "deleted":
                        messages.append(f'{err.expected} manquant')
                    elif err.status == "inserted":
                        messages.append(f'{err.predicted} ajouté')
            if word_uncertain:
                messages.append("articulation incertaine")
            if is_crushed:
                messages.append("mot écrasé ou peu net")

            message = f'Le mot "{span["word"]}" pose problème : ' + ", ".join(messages)
            is_correct = False
...
def build_clean_report(words: list[WordResult]) -> list[ReportItem]:
    report: list[ReportItem] = []
        ...
        if not word.problem_positions:
            report.append(
                ReportItem(
                    ...
                    severity="low",
                    message=f'Le mot "{word.word}" est globalement compris, mais peu net.'
                )
            )
            continue

        for problem in word.problem_positions:
            if problem.issue == "wrong_phoneme":
                severity = "high"
                message = (
                    f'Le mot "{word.word}" pose problème sur le phonème "{problem.phoneme}". '
                    f'Le son semble remplacé par un autre.'
                )
            elif problem.issue == "missing_or_swallowed":
                severity = "high"
                message = (
                    f'Le mot "{word.word}" pose problème sur le phonème "{problem.phoneme}". '
                    f'Ce son semble manquant ou avalé.'
                )
            elif problem.issue == "extra_sound":
                severity = "medium"
                message = (
                    f'Le mot "{word.word}" contient un son supplémentaire autour du phonème "{problem.phoneme}".'
                )
            elif problem.issue == "unclear_or_crushed":
                severity = "low"
                message = (
                    f'Le mot "{word.word}" est globalement compris, mais le phonème "{problem.phoneme}" reste peu net.'
                )
            elif problem.issue == "word_crushed":
                severity = "medium"
                message = (
                    f'Le mot "{word.word}" semble globalement écrasé ou mal articulé.'
                )
            else:
                severity = "medium"
                message = (
                    f'Le mot "{word.word}" contient un problème de prononciation.'
                )
...
def compute_final_decision(words: list[WordResult], score: float) -> FinalDecision:
    has_hard_error = any(
        any(e.type in {"mismatch", "deleted", "inserted"} for e in w.errors)
        for w in words
    )
    has_uncertain = any(
        any(e.type == "uncertain" for e in w.errors)
        for w in words
    )
    has_crushed = any(w.is_crushed for w in words)

    if has_hard_error:
        return FinalDecision(
            status="incorrect",
            message="La prononciation contient au moins une erreur claire."
        )

    if has_crushed or has_uncertain:
        return FinalDecision(
            status="acceptable",
            message="La prononciation est globalement compréhensible, mais certains sons restent peu nets ou incertains."
        )

    return FinalDecision(
        status="correct",
        message="La prononciation semble correcte."
    )
...
async def analyze(
    ...
    report = build_clean_report(words)
    ...
    final_decision = compute_final_decision(words, pronunciation_score)
    is_correct = final_decision.status == "correct"
...
if __name__ == "__main__":
    ...
    print("Test phrase:", text_to_phonemes("elle colorie la vache"))