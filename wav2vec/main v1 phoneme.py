...
app = FastAPI(title="French Pronunciation Analyzer V6.1", version="6.1")
...
def build_word_results(
    ...
    for span in spans:
        ...
        crushed_reasons = []
        if duration_sec > 0 and duration_sec / phoneme_count < MIN_WORD_DURATION_PER_PHONEME:
            crushed_reasons.append("too_short")
        if mean_energy < LOW_ENERGY_THRESHOLD:
            crushed_reasons.append("low_energy")
        if len(word_uncertain) >= 1:
            crushed_reasons.append("uncertain_phonemes")
        if weak_count >= 1:
            crushed_reasons.append("weak_phonemes")

        is_crushed = len(crushed_reasons) > 0 and (clarity_score < 85 or completeness_score < 100)
...
def compute_is_correct(mode: str, errors: list[ErrorItem], score: float, words: list[WordResult]) -> bool:
    if any(w.is_crushed for w in words):
        return False
    if any(not w.is_correct for w in words):
        return False

    if mode == "word":
        return len(errors) == 0 and score >= 95.0
    return len(errors) == 0 and score >= 92.0


def build_global_message(mode: str, score: float, words: list[WordResult]) -> str:
    wrong_words = [w.word for w in words if not w.is_correct]
    wrong_words = list(dict.fromkeys(wrong_words))

    crushed = [w.word for w in words if w.is_crushed]
    crushed = list(dict.fromkeys(crushed))

    if not wrong_words and not crushed:
        return "La prononciation semble correcte."

    if crushed:
        return (
            f'Le mot ou les mots suivants semblent écrasés ou peu nets : {", ".join(crushed)}. '
            f'Score global : {score}%.'
        )

    return (
        f'La prononciation contient des problèmes sur : {", ".join(wrong_words)}. '
        f'Score global : {score}%.'
    )
...
async def analyze(
    ...
    is_correct = compute_is_correct(mode, errors, pronunciation_score, words)
    message = build_global_message(mode, pronunciation_score, words)
