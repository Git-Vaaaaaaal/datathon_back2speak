...
# si un phonème est correct + très confiant, un doute acoustique seul ne doit pas invalider le mot
...
app = FastAPI(title="French Pronunciation Analyzer V8.1", version="8.1")
...
def should_keep_word_valid(
    word_items: list[PhonemeAnalysisItem],
    word_errors: list[PhonemeAnalysisItem],
    word_uncertain: list[PhonemeAnalysisItem],
    is_crushed: bool,
) -> bool:
    if word_errors:
        return False
    ...
    if len(word_uncertain) == 1:
        ...
        if (
            ...
            and p.confidence >= HIGH_CONFIDENCE_VALIDATION_THRESHOLD
        ):
            return True
...
def build_word_results(
    ...
    for span in spans:
        ...
        is_valid = should_keep_word_valid(
            word_items=word_items,
            word_errors=word_errors,
            word_uncertain=word_uncertain,
        ...
        if is_valid:
            ...
            if word_uncertain:
                warnings.append("léger doute acoustique")
                # on garde l'info technique, mais on ne rejette pas le mot
                problem_positions = []
        else:
            reasons = []
        ...
        results.append(
            WordResult(
                ...
                errors=[
                    ...
                    for p in word_items
                    if (p.status in {"mismatch", "deleted", "inserted"})
                    or (p.status == "uncertain" and not is_valid)
                ],
                ...
                is_valid=is_valid,
                is_crushed=is_crushed if not is_valid else False,
                score=overall_score,
...
def build_summary(
    ...
    uncertain = sum(
        ...
        if p.status == "uncertain" and any((not w.is_valid) and (w.word_index == p.word_index) for w in words)
    )
